...
class AVLTree
{
	...
	Info _Big(Node*node)const
	{
		if (!node)
			return 0;
		else
		{
			size_t temp = std::max(_Big(node->left), _Big(node->right));
			return std::max(temp, node->info);
		}
	...
	{
		// find the correct postion && insert the node
		if (node == nullptr)
		...
		// update the balance factor of each node && balance the tree
...
public:
	...
	void dd(vector<pair<string, size_t>>& result, const Info& key, Node* node)const {
		if (node == nullptr) {
			return;
		}
		if (node->info == key) {
		...
		dd(result, key, node->left);
		dd(result, key, node->right);

	}
	void findBigInfo(vector<pair<string, size_t>>& result) const {
		Info big = _Big(root);
		dd(result, big, root);
	}
